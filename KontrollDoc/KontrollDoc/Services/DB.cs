...
#if __ANDROID__ || NETSTANDARD2_0_OR_GREATER || __IOS__
...
/*#else
        ...
        System.Security.SecureString _jelszo;
#endif*/
...
#if __ANDROID__ || NETSTANDARD2_0_OR_GREATER || __IOS__
        Beallitasok _alap = new Beallitasok();
#endif

        long _ABId;
        long _ABFelhasznaloId;
        long _FelhasznaloId;
        bool _securePasswordStore;
        string _alkalmazasKod = "";
        bool _silent = false;
        bool _passThrough = false;
        string _felhasznaloNev;
        DateTime _kapcsolodasIdeje;
        bool _azureKapcsolat;
        ...
        /*public DB(FIT_Common.IDB forras)
        {
            new DB((DB)forras);
        }*/
        ...
        public DB(string alkalmazasKod, string szerver, string adatbazis, string felhasznalo, string jelszo)
        {
            _alkalmazasKod = alkalmazasKod;
            _securePasswordStore = true;
            _azureKapcsolat = AzureEllenorzes();
            _passThrough = true;
            _felhasznaloNev = felhasznalo;
            _jelszo = jelszo;
            _FelhasznaloId = LoginPassThrough(felhasznalo, jelszo);
            if (_FelhasznaloId>0)
            {
                cnAB= new SqlConnection(new SqlConnectionStringBuilder { DataSource = szerver, InitialCatalog = adatbazis, UserID = felhasznalo, Password = jelszo }.ToString());
            }
        }
        ...
        private SqlConnection GetConnectionAzure()
        {
            ...
            {
//#if __ANDROID__ || __IOS__
                if (string.IsNullOrEmpty(pwdAzure))
                {
                    pwdAzure="";
                    pwdAzure+='o';
                    pwdAzure+='4';
                ...
                SqlConnectionStringBuilder cnsb = new SqlConnectionStringBuilder
                {
                    ...
                    Encrypt = true,
                    ConnectTimeout = 30,
                    ApplicationName="FIT_Common_WEB" + (_alkalmazasKod==""?"":"-"+_alkalmazasKod),
                    UserID = "KontrollUser",
                    Password = pwdAzure
                };
...
//#if __ANDROID__ || __IOS__
            ...
            return new SqlConnection { ConnectionString = cnsb.ConnectionString };
/*#else
            SqlConnectionStringBuilder cnsb = new SqlConnectionStringBuilder
            {
                DataSource = server,
...
//#if __ANDROID__ || __IOS__
                ...
                cnAzure = new SqlConnection { ConnectionString = cnsb.ConnectionString };
/*#else
                SqlConnectionStringBuilder cnsb = new SqlConnectionStringBuilder
                {
                    DataSource = server,
...
//#if __ANDROID__ || __IOS__
                SqlConnectionStringBuilder cnStrBuilder = new SqlConnectionStringBuilder() { DataSource = serverSQL, InitialCatalog = prms[3].Value.ToString(), TrustServerCertificate = true, Encrypt = true, ApplicationName = "Kontroll2015" + (_alkalmazasKod == "" ? "" : "-" + _alkalmazasKod), UserID = _felhasznaloNev, Password = _jelszo };
                cnAB = new SqlConnection(cnStrBuilder.ConnectionString);
/*#else
                ...
                //try
                //{
                //    _ABFelhasznaloId = GetDolgozoAz(cnAB, prms[6].Value.ToString());
                //}
                //catch (SqlException e)
                //{
                //    if (e.Number == 2812)//Could not find stored procedure.
                //    {
        ...
        private SqlConnection GetClientConnectionKontrollVezerlo(long AdatbazisId,long FelhasznaloId)
        {
            if (AdatbazisId != _ABId || cnAB == null || _kapcsolodasIdeje.Year != DateTime.Now.Year)
            {
                ...
                */
                ...
                if (_passThrough)
//#if __ANDROID__ || __IOS__
                    cnAB = new SqlConnection(cnStrBuilder.ConnectionString + "; user=" + _felhasznaloNev + "; password=" + _jelszo);
/*#else
                    cnAB = new SqlConnection(cnStrBuilder.ConnectionString, new SqlCredential(_felhasznaloNev, _jelszo));
#endif*/
                else
        ...
        private string ElerhetoSzerver()
        {
            ...
            catch (Exception e) { server = "sql.fit.hu"; }
            if (server=="")
                try
                {
        ...
        private void GetKontrollVezerloFromAzure(out string server,out string adatbazis)
        {
            ...
            try
            {
                ...
                if (s.HelyiKontrollVezerloSzerver != server || s.HelyiKontrollVezerloAdatbazis != adatbazis)
                {
                    s.HelyiKontrollVezerloSzerver = server;
                    s.HelyiKontrollVezerloAdatbazis = adatbazis;
                    s.Save();
                }
            }
            catch
        ...
        private Int32 GetDolgozoAz(SqlConnection cn,string userNev)
        {
            List<SqlParameter> prms = new List<SqlParameter>
            {
                new SqlParameter { ParameterName="@UserNev", SqlDbType=SqlDbType.NVarChar, Size=50, Value=userNev }
            };
            return Convert.ToInt32(_ExecuteSP(cn, "GetDolgozoAzFromUserNev", prms, _FelhasznaloId));
        }
        ...
        /*public bool SetLoginData(IDB forras)
        {
            return SetLoginData((DB)forras);
        }*/
        ...
        public long Login(string FelhasznaloiNev, string Jelszo)
        {
            ...
            List<SqlParameter> prms = new List<SqlParameter>()
            {
                ...
                new SqlParameter() { ParameterName="@Jelszo", SqlDbType=SqlDbType.NVarChar, Size=255, Value=Jelszo.SHA512UnicodeHash() },
